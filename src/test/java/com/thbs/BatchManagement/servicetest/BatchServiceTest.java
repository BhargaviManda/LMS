...
public class BatchServiceTest {
    ...
    public void testCreateBatch_Success() {
        ...
        // Verify the properties of the saved batch
        verify(batchRepository).save(argThat(b -> 
            b.getBatchName().equals(batch.getBatchName()) &&
            b.getBatchDescription().equals(batch.getBatchDescription()) &&
            b.getStartDate().equals(batch.getStartDate()) &&
            b.getEndDate().equals(batch.getEndDate()) &&
            b.getBatchSize().equals(batch.getBatchSize()) &&
            b.isLearningPlan() == false
        ));
    }
    ...
    @Test
    public void testGetBatchName_Success() {
        // Mock data
        Long batchId = 1L;
        Batch batch = new Batch();
        batch.setBatchId(batchId);
        ...
        // Mock the behavior of findById() method in the BatchRepository
        when(batchRepository.findById(batchId)).thenReturn(Optional.of(batch));

        // Perform the service method
        Map<String, String> result = batchService.getBatchName(batchId);

        // Expected result
        Map<String, String> expectedBatchDetails = new HashMap<>();
        expectedBatchDetails.put("batchName", "Test Batch");

        // Verify the result
        assertEquals(expectedBatchDetails, result);
    }

    
    @Test
    public void testGetBatchName_BatchNotFoundException() {
        // Mock data
        Long batchId = 1L;

        // Mock the behavior of findById() method in the BatchRepository to return empty Optional
        when(batchRepository.findById(batchId)).thenReturn(Optional.empty());

        // Perform the service method and expect an exception
        Exception exception = assertThrows(BatchNotFoundException.class, () -> {
            batchService.getBatchName(batchId);
        });
